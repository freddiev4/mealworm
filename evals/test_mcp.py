    ...
    try:
        ...
        if search_tool:
            result = await search_tool.ainvoke(
                {"query": "meal", "filter": {"property": "object", "value": "page"}}
            )
        ...
        if db_query_tool:
            print(
                "✅ API-post-database-query tool found (would need a real database ID to test)"
            )
        else:
        ...
        if page_tool:
            print(
                "✅ API-retrieve-a-page tool found (would need a real page ID to test)"
            )
        else:
